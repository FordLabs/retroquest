...
describe('Radiator Page', () => {
	it('should get active action items and display them on page', async () => {
		...
		expect(
			screen.getByText("Take a look at all your team's active action items")
		).toBeDefined();
		...
		expect(
			screen.getByText(moment(activeActionItem.dateCreated).format('MMM Do'))
		).toBeDefined();
		...
		expect(
			screen.queryByText(
				moment(completedActionItem.dateCreated).format('MMM Do')
			)
		).toBeNull();
	});
	...
	it('should show "No active action items" message when no active action items are present', () => {
		...
		expect(description.innerHTML).toBe(
			'You can create new action items on the <span class="bold">retro page</span>.'
		);
