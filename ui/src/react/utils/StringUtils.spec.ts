 ...
 */
import { validateConfirmationPassword, validatePassword, validateTeamName } from './StringUtils';
...
describe('String Utils', () => {
  ...
  it.each([
    ...
    ['1', undefined],
    ['a1', undefined],
  ])('validating team name "%s" should return error message "%s"', (teamName, errorMessage) => {
  ...
  ])('validating password %s should return error message "%s"', (password, errorMessage) => {
    expect(validatePassword(password)).toBe(errorMessage);
  });

  const misMatchingPasswords = 'Please enter matching passwords';

  it.each([['CorrectPassword1', 'WrongConfirmationPassword', misMatchingPasswords]])(
    ...
    (password, confirmationPassword, errorMessage) => {
      expect(validateConfirmationPassword(password, confirmationPassword)).toBe(errorMessage);
    }
  );
});
