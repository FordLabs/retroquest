...
describe('EditableText', () => {
  ...
  describe('when not editing and selectable', () => {
    beforeEach(() => {
      render(<EditableText value={'sample text'} editing={false} selectable={true} onSelect={mockSelect} />);
    });
  ...
  describe('when disabled', () => {
    beforeEach(() => {
      render(<EditableText value={'sample text'} editing={false} disabled={true} />);
    });
    ...
    it('is is translucent', () => {
      expect(screen.getByTestId('editableText-container').className).toContain('disabled');
    });
  });

  describe('when not selectable', () => {
    beforeEach(() => {
      render(<EditableText value={'sample text'} editing={false} selectable={false} />);
    });
...
function textIsReadonlyAndDisabled(readonlyAndDisabled: boolean) {
  ...
  expect(textArea.getAttribute('readonly')).toBe(readonlyAndDisabled ? '' : null);
  expect(textArea.getAttribute('disabled')).toBe(readonlyAndDisabled ? '' : null);
}
